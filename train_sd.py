...
from model.uien import UIEN_sd
from utils.loader import NYUv2_sd
...
def main():
    ...
    train_set = NYUv2_sd(dir='/home/lilka/NAS_AIlab_dataset/dataset/NYU2v', mode='train', trans=transforms.ToTensor(), gt_crop_size=None)
    training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, \
        batch_size=opt.batchSize, shuffle=True)
    test_set = NYUv2_sd(dir='/home/lilka/NAS_AIlab_dataset/dataset/NYU2v', mode='val', trans=transforms.ToTensor(), gt_crop_size=None)
...
def train(training_data_loader, optimizer, model, criterion, epoch):
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        input, single_distorted_input, target, img_name = batch[0].float(), batch[1], batch[2].float(), batch[3]

        if opt.cuda:
            target = target.cuda()

        _, single_distorted_output = model(single_distorted_input[opt.mode].cuda(), opt.mode)

        loss = criterion(single_distorted_output, target)

        single_distorted_output = single_distorted_output.cpu().data.numpy().transpose(0, 2, 3, 1).astype(np.float32)
        target = target.cpu().data.numpy().transpose(0, 2, 3, 1).astype(np.float32)

        ssim_result = SSIM_RGB(single_distorted_output, target, shave_border=5)

        output_y = RGB2YCBRC(single_distorted_output)
...
def eval(test_data_loader, model, criterion):
    ...
    samples_dir_name = os.path.join('result', 'each',  num2mode(opt.mode), 'samples')
    ...
    for iteration, batch in enumerate(test_data_loader): 
        ...
        ssim_predicted = SSIM_RGB(single_distorted_output, target, shave_border=5)
        ...
        if iteration % 10 == 0:
            cv2.imwrite(os.path.join(samples_dir_name, img_name[0].split('/')[-1].split('.')[0] + '_' + str(psnr_predicted) + '.jpg'), single_distorted_output[0]*255)
...
def save_checkpoint(model, epoch):
    model_out_dir_path =os.path.join('result', 'each', num2mode(opt.mode), 'checkpoint')
...
def save_graph(loss, psnr, ssim, epoch, mode):
    graph_path_dir = os.path.join('result', 'each',  num2mode(opt.mode), 'graph', f'{num2mode(opt.mode)}')
