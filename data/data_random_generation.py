...
def main():
    ...
    opt.rain = random.choice([True, False])
    opt.haze = random.choice([True, False])
    opt.downscale = random.choice([True, True])
    opt.blur = random.choice([True, False])
    opt.noise = random.choice([True, False])
    opt.beta = random.uniform(0.4, 2.4) 
    opt.a = random.uniform(0.5, 1.0)
    opt.kernelsize = random.choice([3, 5, 7])
    opt.angle = random.randint(0, 180)
    opt.var = random.uniform(0.005, 0.025)
    ...
    print(len(glob.glob(os.path.join(root, dir_name+'/*'))))
